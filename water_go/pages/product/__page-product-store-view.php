 ...
 */
   ...
   GLOBAL $wpdb;
   $get_category = "SELECT * FROM wp_watergo_product_category WHERE name NOT IN ('Đá cây') ";
   $res_category = $wpdb->get_results($get_category);
   ...
   <div v-show='loading == false' class='page-product-store-view'>
      ...
      <div class='inner'>
         <div class='product-store-view-form'>
            ...
            <!-- ICE CATEGORY PARENT -->
            <div v-show='product_type == "ice" && allow_category_parent == true' class='form-control form-select'>
               <div class='form-title'><?php echo __('Type Of Ice', 'watergo'); ?></div>
               <select v-model='select_category_parent'>
                  <option :value="{ value: 0 }" disabled><?php echo __('Select Category', 'watergo'); ?></option>
                  <option 
                     v-for='(cat_parent, cat_parent_key) in get_category_parent' :index='cat_parent_key'
                     value="cat_parent.id">
                     {{ cat_parent.name }}
                  </option>
               </select>
            ...
            <!-- NAME * DEVICE -->
            <div v-show='is_category_device == true' class='form-control form-select'>
            ...
            <div v-show='product_type == "ice"' class='form-control'>
               ...
               <input inputmode='numeric' class='input-trailing' data-trailing='kg' type="text" placeholder='<?php echo __('Input Weight', 'watergo'); ?>'>
            </div>

            <div v-show='product_type == "ice"' class='form-title small-size'><?php echo __('Length * Width', 'watergo'); ?></div>
            <div v-show='product_type == "ice"' class='form-control'>
...
var app = Vue.createApp({
   data (){
      ...
      return {
         ...
         select_category_parent: { value: 0 },
   ...
   computed: {
      ...
      get_category_parent(){
         if( this.product_type == 'ice' ){
            return this.category_parent.filter( cat => cat.category == 'type_of_ice' && cat.parent == this.select_category.value );
         }
      },

      allow_category_parent(){
         return this.category.some( item => item.id == this.select_category.value && item.name != "Thiết bị đá" );
      },

      is_category_device(){
         var _is_device_ice   = this.category.some( item => item.id == this.select_category.value && item.name == "Thiết bị đá" );
         var _is_device_water = this.category.some( item => item.id == this.select_category.value && item.name == "Thiết bị nước" );
         if(this.product_type == 'ice' && _is_device_ice == true ) return true;
         if(this.product_type == 'water' && _is_device_water == true ) return true;
         return false;
      },
      ...
      async btn_action_product( event ){
         ...
         */
         if( this.product_type == 'ice' && this.is_can_action == true ){
            ...
            form.append('ice_weight', parseInt(this.ice_weight ));
      ...
      async get_product(product_id){
         ...
         if( r != undefined ){
            ...
            if( res.message == 'product_found' ){
               ...
               this.ice_weight               = this.product.weight;
   ...
   async created(){
      ...
      $('.input-trailing').on('input', function() {
         ...
         appInstance.ice_weight = inputValue;
      });
