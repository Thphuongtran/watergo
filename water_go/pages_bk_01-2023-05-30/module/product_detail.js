const PageProductDetail = {
   ...
   template: `
<div v-if='
   $root.navigator == "product-detail" && 
   loading == false && 
   product_detail != null && 
   store != null' class='page-product-detail'>
   ...
   <div class='product-detail-wrapper'>
      <div class='product-header'>
         ...
         <div class='main'> 
            <img :src="get_image_product(product_detail.id)">
            <span v-if='has_discount(product_detail) == true' class='badge-discount bottom left'>-{{ product_detail.discount_percent }}%</span>
            <span v-if='product_detail.stock == 0' class='badge-discount bottom right'>Out of Stock</span>
         </div>
      </div>

      <div class='inner'>
         <div class='product-design product-detail'>
            <p class='tt01'>{{ product_detail.name }}</p>
            <p class='tt02'>{{ get_product_quantity(product_detail) }}</p>
            <p class='tt03'> {{ product_detail.description }}</p>
            <div class='gr-price' :class="has_discount(product_detail) == true ? 'has_discount' : '' ">
               <span class='price'>
                  {{ has_discount(product_detail) == true 
                     ? common_get_product_price(product_detail.price, product_detail.discount_percent) 
                     : common_get_product_price(product_detail.price)
                  }}
               </span>
               <span v-if='has_discount(product_detail) == true' class='price-sub'>{{ common_get_product_price(product_detail.price) }}</span>
            </div>
            <div class='entry-quantity'>
               ...
               <div class='quantity-event'>
                  ...
                  <input type="number" v-model='product_quantity_count' :max='product_detail.stock' disabled>
                  <span @click='plusQuantity(product_detail.stock)'>
      ...
      <div class='inner'>
         <div v-if='store != null' class='list-tile shop-card'>
            ...
            <div class='leading'>
               <div class='avatar'><img :src="get_image_store(store.id)"></div>
            </div>
            <div class='content'>
               ...
               <div v-if='check_store_time_working(store.start_time, store.close_time) == true'>
            ...
            <div class='actions'><button @click='$root.gotoPage("store-detail", { back: "product-detail", product_id: this.product_detail.id, store_id: this.store.id })' class='btn-text'>View Store</button></div>
         </div>
      </div>
      ...
      <div v-if='product_by_store.length > 0' class='inner space-top-product'>
         ...
         <div class='list-horizontal'>
            <ul>
               <li @click='findProduct(product.id)' v-for='(product, index) in product_by_store' class='product-design'>
                  <div class='img'>
   ...
   <div class='product-detailt-bottomsheet'>
      ...
      <button @click='addToCart' class='btn-text'>Add to Cart</button>
      <button @click='gotoPageOrder' class='btn-primary' :class='product_quantity_count == 0 ? "disabled" : "" '>Order</button>
   </div>
   ...
   methods: {
      ...
      addToCart(){
         if( this.product_quantity_count > 0 ){
            var _storeExists = this.$root.carts.some( item => item.store_id == this.store.id );
            if( _storeExists == false ){
               this.$root.carts.push({
                  ...
                  products: [
                     { 
                        ...
                        product_name: this.product_detail.name,
                        product_stock: this.product_detail.stock,
                        product_price: this.product_detail.price,
                        product_discount_percent: this.product_detail.discount_percent,
               ...
               this.$root.carts.forEach(item => {
                  if (item.store_id == this.store.id) {
                     var product = item.products.find(product => product.product_id == this.product_detail.id);
                     if (product) {
                        product.product_quantity_count = this.product_quantity_count;
                     }else{
                        item.products.push({ 
                           product_id: this.product_detail.id, 
                           product_quantity_count: this.product_quantity_count,
      ...
      // MAKE ORDER ADD TO CART
      // GO TO PAGE ORDER
      gotoPageOrder(){
         if( this.product_quantity_count > 0 ){
            this.addToCart();
            this.$root.navigatorHistory.push({ 
               to: 'order',
               transport: { back: 'product-detail', product_id: this.product_detail.id } 
            });
            this.$root.navigator = "order";
         }
      },

      timestamp_to_date(timestamp){
         var _time = new Date(timestamp * 1000);
         return _time.getDate() + '/' + _time.getMonth() + '/' + _time.getFullYear();
      },

      check_store_time_working(_startTime, _closeTime){
         if( _startTime == 0 || _closeTime == 0 ) return false;
         var startDate = new Date(_startTime);
         var closeDate = new Date(_closeTime);
         var currentDate = new Date();
         if ( currentDate >= closeDate && currentDate < startDate ) { return false;
         } else { return true; }
      },

      check_product_exists_in_root(product_id){
         return this.$root.products.forEach((item) => {
      ...
      async findProduct( product_id ){
         ...
         this.store = null;
         this.product_by_store = [];
         
         this.loading = true;

         var _productExistsInStore = this.$root.stores.some(item => {
            if(item.hasOwnProperty('products') == false ) return false;
            return item.products.some( p => p.id == product_id);
         });

         if( _productExistsInStore == false ){
            ...
            this.$root.stores.some(item => {
               this.product_detail = item.products.find( p => p.id == product_id );
            });
         }
         await this.findStore(this.product_detail.store_id);
         await this.findRelatedProductInStore(this.store.id, product_id);
         this.find_product_quantity_count( this.store.id, product_id);
         this.loading = false;
      },
      ...
      async findStore( store_id ){
         ...
         if(_checkStore == true ){
            ...
            this.$root.stores.some( store => {
               if( store.id == store_id ){
                  var _productInStoreYet = store.products.some( product => product.id == this.product_detail.id);
                  if(_productInStoreYet == false ){
                     store.products.push( this.product_detail);
                  }
               }
            });
         }else{
            ...
            if( r != undefined ){
               ...
               if(res.message == 'store_found' ){
                  this.store = res.data;
                  res.data.products = [];
                  res.data.products.push( this.product_detail);
      ...
      async findRelatedProductInStore( store_id, product_id ){
         this.$root.stores.forEach( async ( store ) => {
            if( store.id == store_id && store.products.length < 11 ){
               var form = new FormData();
               ...
               if(this.product_by_store.length > 0 ){
                  this.product_by_store.forEach((product) => {
                     var lookup = store.products.some((p) => p.id == product.id);
                     if( lookup == false ){
               ...
               // load cache from stores
               if( store.id == store_id ){
                  store.products.forEach((product) => {
                     if( product.id != product_id ){
                        this.product_by_store.push(product);
                     }
                  });
               }
            }
      ...
      plusQuantity( stock ){
         if( this.product_quantity_count < stock ){ this.product_quantity_count++; }
      },
