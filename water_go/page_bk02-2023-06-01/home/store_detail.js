   ...
   <div class="inner">
      ...
      <div v-if='reviews.length > 0' class='store-review'>
         <div class='review-head'>
            ...
            <span class='rating-average'>{{get_review_rating_average}}</span>
            <span class='total-review'>( {{total_reviews}} reviews)</span>
            <button class='btn-action' @click='$root.gotoPage("review", {back: "store-detail", store_id: store.id})'>See all</button>
         </div>
      ...
      <div v-if='products.length > 0' class="store-product mt30">
         ...
         <div v-if='products.length > 0' class='grid-masonry'>
            ...
            <div 
               ...
               v-for='(product, index) in get_product_filter ' :key='index'>
               <div class='img'>
...
<div v-if="loading == true">
   ...
   methods: {
      ...
      renew(){
         ...
         this.rating_average = [];
         this.store_type = [];
         this.select_product_type_value = '';
      },
      ...
      async findStore( store_id ){
         ...
         if( this.store.store_type == "both" ){
            ...
            this.products.some((product) => {
               if(product.product_type == 'water'){
                  this.store_type.some( type => { 
                     if( type.value == 'water' ){
                        type.active = true;
                        // this.select_product_type_value = 'water';
                     }
                  });
               }else if(product.product_type == 'ice'){
                  this.store_type.some( type => { 
                     if( type.value == 'ice' ){
                        type.active = true;
                        // this.select_product_type_value = 'water';
      ...
      select_product_type( type ){
         this.select_product_type_value = type;
      }
   ...
   computed: {
      ...
      get_product_filter(){
         return this.products.filter( product => {
            var _findType = this.store_type.some( type => product.product_type == type.value );
            if(_findType){
               return product;
            }
         });
      ...
      get_review_rating_average(){
         var averageRating = 0;
         if( this.reviews.length > 0 ){
            var totalRating = this.reviews.reduce((sum, review) => sum + parseFloat(review.rating), 0);
