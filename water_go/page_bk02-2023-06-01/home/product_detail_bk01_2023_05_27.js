const PageDetail = {
      ...
      <div v-if='
         ...
         <div class='product-detail-wrapper'>
               ...
               <div class='product-design product-detail'>
                  ...
                  <div class='entry-quantity'>
                     ...
                     <div class='quantity-event'>
                        ...
                        <input type="number" v-model='quantity' :max='product_detail.stock' disabled>
            ...
            <div class='inner space-top-product'>
               ...
               <div v-if='product_by_store.length > 0' class='list-horizontal'>
                  <ul>
                     <li @click='$root.gotoPage("order")' v-for='(product, index) in product_by_store' class='product-design'>
                        <div class='img'>
         ...
         <div class='product-detailt-bottomsheet'>
            ...
            <button @click='gotoPageOrder' class='btn-primary' :class='quantity == 0 ? "disabled" : "" '>Order</button>
         </div>
   ...
   methods: {
      ...
      addToCart(){
         if( this.quantity > 0 ){
            var _checkCarts = this.$root.carts.some( (item) => item.id == this.product_detail.id );
            if( _checkCarts ){
               this.$root.carts.forEach((e) => {
                  if( e.id == this.product_detail.id ){
                     e.product_quantity = this.quantity
                  }
               });
            }else{
               this.$root.carts.push( {
                  product: this.product_detail,
                  product_quanity: this.quantity,
                  store: this.store
               });
               // this.$root.carts.push( {
               //    id: this.product_detail.id,
               //    product_name: this.product_detail.name,
               //    product_quantity_count: this.quantity,
               //    product_quantity: this.get_product_quantity(this.product_detail),
               //    product_price: this.product_detail.price,
               //    product_discount: this.product_detail.discount_percent,
      ...
      async findProduct( product_id ){
         ...
         // var _checkExists = this.$root.products.some( item => item.id == product_id);
            // this.product_detail = this.$root.products.find(item => item.id == product_id );
         // if( _checkExists == true ){
            // if cart already exists?
            // var _checkExistsInCart = this.$root.carts.some( item => item.id == product_id);
         ...
         //          this.$root.products.push( res.data );
         //       }
         //    }

         // }

         // await this.findRelatedProductByStore(this.product_detail.store_id, product_id);
      ...
      async findRelatedProductByStore( store_id, product_id ){
         // find store first
         var _localProducts = [];
         this.$root.products.forEach((item ) => {
            if( item.store_id == store_id && item.id != product_id ){
               _localProducts.push( item );
            }
         });

         // limit item 10
         if( _localProducts.length >= 10 ){
            this.product_by_store.push( ..._localProducts);
         }else{
            var form = new FormData();
            ...
            if( r != undefined ){
               ...
               if( res.message == 'product_found' ){
                  ...
                  res.data.forEach((item) => {
                     var _check = this.check_product_exists_in_root(item.id);
                     if( _check == false ){
                        this.$root.products.push( p );
                     }
                  });
                  // save to root
               }
