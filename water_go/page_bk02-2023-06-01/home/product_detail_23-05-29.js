   ...
   <div class='product-detail-wrapper'>
      ...
      <div class='inner'>
         <div v-if='store != null' class='list-tile shop-card'>
            ...
            <div class='actions'><button class='btn-text'>View Store</button></div>
         </div>
      </div>
   ...
   methods: {
      ...
      addToCart(){
         if( this.product_quantity_count > 0 ){
            this.$root.stores.some( ( item, index ) => {
               if( item.id == this.store.id ){
                  var _productInCarts = item.carts.some( p => p == this.product_detail.id);
                  if(_productInCarts == false ){
                     item.carts.push( parseInt( this.product_detail.id ) );
                     // update product count
                     item.products.some( p => {
                        if( p.id == this.product_detail.id ){
                           p.product_quantity_count = this.product_quantity_count
                        }
                     });
                  }else{
                     // update product count
      ...
      async findProduct( product_id ){
         ...
         this.product_quantity_count = this.product_detail.product_quantity_count;
      ...
      async findStore( store_id ){
         ...
         if(_checkStore == true ){
            ...
            this.$root.stores.some( s => {
               if( s.id == this.store.id ){
                  var _productInStoreYet = s.products.some( p => p.id == this.product_detail.id);
                  if(_productInStoreYet == false ){
                     s.products.push( this.product_detail);
                  }
               }
            });
         }else{
