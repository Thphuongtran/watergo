const PageOrder = {
   name: 'PageOrder',
   template: `
<div v-if='$root.navigator == "order" && showBannerOrderSuccess == false' class='page-product-order'>
      ...
      <div @click='$root.gotoPage("delivery_address", { back: "order"})' class='list-tile delivery-address'>
         <div class='content'>
            ...
            <div v-else>
               ...
               <pPageOrder class='tt02'>{{ $root.delivery_address_primary.name }} | {{ $root.delivery_address_primary.phone }}</p>
            </div>
         </div>
         <div class='action'>
   ...
   <ul class='list-tile order'>
      <li  
         v-for='(store, index) in $root.carts' :key='index'>
         ...
         <div 
            ...
            <div v-if="product.product_select == true" class="list-items-wrapper">
               ...
               <div class='order-price'>
                  ...
                  <span class='od-price-discount' v-if='product.product_discount_percent != null'>
   ...
   <div class='product-detailt-bottomsheet cell-placeorder'>
      <p class='price-total'>Total: <span class='t-primary t-bold'>{{ $root.count_product_total_price.price_discount }}</span></p>
      <button @click='buttonPlaceOrder' class='btn-primary' :class="checkUserCanOrder == false ? 'disabled' : '' ">Place Order</button>
   </div>
</div>

<div v-if='showBannerOrderSuccess == true' class='banner'>
   ...
   <div class='banner-footer'>
      <button @click='gotoHome' class='btn btn-outline'>Exit</button>
   </div>
</div>
   `,
   data(){
   ...
   methods: { 
      ...
      reset_delivery_time(){          
         this.deliverySelectType = {
            once: false,
            timeOnce: { immediately: false, selectDate: false },
            weekly: false,
            monthly: false,
         };
         this.deliveryTimeData = {
            once: { date: null, time: null },
            weekly: [],
            monthly: []
         };
         this.deliveryWeeklyDomIncrement = 0;
         this.deliveryWeeklyOrder = 0;
         this.deliveryMonthlyDomIncrement = 0;
         this.deliveryMonthlyOrder = 0;
         this.deliveryEverymonthOrder = 0;
      ...
      gotoHome(){
         this.$root.gotoHome();
         this.showBannerOrderSuccess = false;
      },
      ...
      async buttonPlaceOrder(){
         ...
         _delivery_data.products = [];
         _delivery_data.delivery_address = '';
         _delivery_data.order_time = _currentTimestamp;
         ...
         if( this.deliverySelectType.once == true ){
            ...
            this.deliveryTimeData.weekly.forEach( item => {
               if( item != undefined && item.day != null && item.time != null){
                  _delivery_data.selectDate.push( { day: item.day, time: item.time } );
               }
            });
         }
            ...
            this.deliveryTimeData.monthly.forEach( item => {
               if( item != undefined && item.date != null && item.time != null ){
                  _delivery_data.selectDate.push( { day: item.date, time: item.time } );
               }
            });
         }else{
            _delivery_data = {};
         ...
         if( this.$root.delivery_address_primary != null ){
            _delivery_data.delivery_address = {
               name: this.$root.delivery_address_primary.name,
               phone: this.$root.delivery_address_primary.phone,
               address: this.$root.delivery_address_primary.address
            };
         ...
         this.$root.carts.some( store => {
            store.products.some( product => {
               if( product.product_select == true ){
                  ...
                  _delivery_data.products.push(_obj);
               }
            });
         });
         ...
         if(this.checkUserCanOrder == true ){
            ...
            form.append('action', 'atlantis_order');
            form.append('event', 'add');
            form.append('delivery_data', JSON.stringify(_delivery_data.selectDate) );
            form.append('delivery_address', JSON.stringify(_delivery_data.delivery_address) );
            form.append('delivery_type', _delivery_data.delivery_type );
            form.append('products', JSON.stringify(_delivery_data.products) );
            var r = await window.request(form);
            if( r != undefined ){
               var res = JSON.parse( JSON.stringify( r ));
               if( res.message == 'order_insert_ok' ){
                  this.reset_delivery_time();
                  this.showBannerOrderSuccess = true;
                  
                  for(let i = this.$root.carts.length - 1; i >= 0; i-- ){
                     var _carts = this.$root.carts[i];
                     for( let j = _carts.products.length - 1; j >= 0; j-- ){
                        var _product = _carts.products[j];
                        if( _delivery_data.products.some((deliveryProduct) => deliveryProduct.product_id == _product.product_id )){
                           _carts.products.splice(j, 1)
                        }
                     }
                     // remove store when no product in cart
                     if (_carts.products.length === 0) {
                        _carts.splice(i, 1);
                     }
                  }
