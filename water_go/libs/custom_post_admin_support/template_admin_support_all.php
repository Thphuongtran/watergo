...
function template_admin_support_all(){
      ...
      <table class='wp-list-table widefat fixed striped table-view-list posts'>
         ...
         <tr v-if='supports.length > 0' v-for='(support, index ) in supports ' :key='index'>
            <td class='title column-title has-row-actions column-primary page-title'>
               <a :href=" '?page=admin_support_all&id=' + support.id  + '&action=edit' ">
      ...
      createApp({
         ...
         methods: {
            ...
            async update( ){
               ...
               form.append('action', 'atlantis_update_admin_support');
         ...
         async created(){
            ...
            if( id != undefined && action != undefined && action == 'edit' ){
               var form = new FormData();
               form.append('action', 'atlantis_get_admin_supports');
               form.append('id', id );
               var r = await this.request(form);
               if( r != undefined ){
                  var res = JSON.parse( JSON.stringify( r ));
